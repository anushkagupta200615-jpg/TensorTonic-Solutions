...
class BertEmbeddings:
    ...
    def forward(self, token_ids: np.ndarray, segment_ids: np.ndarray) -> np.ndarray:
        ...
        token_embeds = self.token_embeddings[token_ids]  # (batch, seq_len, hidden_size)

        # Position embeddings lookup
        position_indices = np.arange(seq_len)  # [0, 1, ..., seq_len-1]
        position_embeds = self.position_embeddings[position_indices]  # (seq_len, hidden_size)
        position_embeds = np.tile(position_embeds, (batch_size, 1, 1))  # (batch, seq_len, hidden_size)

        # Segment embeddings lookup
        segment_embeds = self.segment_embeddings[segment_ids]  # (batch, seq_len, hidden_size)
